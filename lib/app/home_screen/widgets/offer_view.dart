...
class OfferView extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    final itemCount = controller.couponRestaurantList.length >= 15 ? 15 : controller.couponRestaurantList.length;
    return SizedBox(
      ...
      child: PageView.builder(
        ...
        itemBuilder: (BuildContext context, int index) {
          ...
          bool endsSoon = offerModel.expiresAt != null && offerModel.expiresAt!.toDate().difference(DateTime.now()).inHours < 24;
          return InkWell(
            ...
            child: Padding(
              padding: const EdgeInsets.symmetric(horizontal: AppThemeData.space4),
              child: ClipRRect(
                    ...
                    Container(
                      decoration: BoxDecoration(
                        gradient: LinearGradient(
                          ...
                          colors: [Colors.black.withValues(alpha: 0), Colors.black.withValues(alpha: 0.8)],
                        ),
                      ),
                    ),
                    if (!isOpen)
                          ...
                          Row(
                            children: [
                              ...
                              if (vendorModel.isSelfDelivery == true && Constant.isSelfDeliveryFeature == true) ...[
                                ...
                                Container(width: 4, height: 4, decoration: const BoxDecoration(shape: BoxShape.circle, color: AppThemeData.grey400)),
                                const SizedBox(width: AppThemeData.space8),
                                SvgPicture.asset("assets/icons/ic_free_delivery.svg", width: 14, height: 14),
                                const SizedBox(width: 4),
                                const TranslatedText(
                                  "Free Delivery",
                                  style: TextStyle(fontSize: 12, fontFamily: 'Urbanist', fontWeight: FontWeight.w500, color: AppThemeData.grey300),
                                ),
                              ],
                            ],
                          ),
                          if (endsSoon)
                            Padding(
                              ...
                              child: Container(
                                ...
                                decoration: BoxDecoration(
                                  color: AppThemeData.danger300.withValues(alpha: 0.9),
                                  borderRadius: BorderRadius.circular(AppThemeData.radius8),
                                ),
                                child: Row(
