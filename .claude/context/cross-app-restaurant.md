...
| 11 | **Display customer-facing payment mode and split tender safely** | Orders may now include `paymentBreakdown` with wallet + COD or wallet + online components. Restaurant order detail should show customer-friendly modes such as Wallet, UPI, Card, Net Banking, or COD and must not expose gateway names to restaurant staff unless an internal support-only panel needs them. Existing orders with only `payment_method` must still render. | High | 2026-06-18 |
| 12 | **Refund/cancellation state should support split payments** | Cancellation views should account for wallet, COD, and online portions separately. Wallet portions may refund instantly; source refunds may be pending/manual review. Avoid assuming every non-COD order refunds the full amount to wallet. | High | 2026-06-18 |
| 13 | **Suggested add-ons v1 requires no menu schema change** | Customer checkout add-ons should use existing product/menu data only. Restaurant app does not need a product schema change for the first add-ons pass. | Low | 2026-06-18 |
